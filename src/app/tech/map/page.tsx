...
export default function TechMap() {
  ...
  if (!mounted || !driverId) return null;
  ...
  const getCoords = (suburb: string): [number, number] => {
    const coords: Record<string, [number, number]> = {
      "Durbanville": [-33.8333, 18.6500],
      "Sea Point": [-33.9167, 18.3833],
      "City Bowl": [-33.9249, 18.4241],
      "Claremont": [-33.9800, 18.4650],
      "Camps Bay": [-33.9500, 18.3833],
      "Green Point": [-33.9067, 18.4167],
      "Rondebosch": [-33.9667, 18.4833],
      "Milnerton": [-33.8667, 18.5000],
      "Woodstock": [-33.9333, 18.4500],
      "Constantia": [-34.0333, 18.4333],
      "Bishopscourt": [-33.9833, 18.4500],
      "Observatory": [-33.9333, 18.4667],
      "Gardens": [-33.9333, 18.4167],
      "Fresnaye": [-33.9250, 18.3833],
    };
    // Add some random jitter so markers don't overlap exactly
              ...
              <Marker key={job.id} position={position} icon={icon}>
                <Popup>
                  <div className="p-2">
                    ...
                    <Button size="sm" className="mt-2 w-full" onClick={() => router.push(`/tech/job/${job.id}`)}>
                      View Job
                    </Button>
                  </div>
                </Popup>
